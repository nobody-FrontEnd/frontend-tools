...
interface uploadfilePropType {
    fileType: 'image';
}

export const useUploadFile = (props: uploadfilePropType) => {
    const { fileType } = props;
    const formData = new FormData();
    const uploadFun = (extraParams?: any): Promise<any> => {
        return new Promise((resolve, reject) => {
            ...
            upload.addEventListener('change', async () => {
                if (upload.files) {
                    if (upload.files.length === 0) reject(new Error('请选择文件'));
                    const file = upload.files[0];
                    if (file.size > 12582912) reject(new Error('图片大小必须是12MB以内'));
                    //   const formData = new FormData();
                    const uploadParams = {
                        file,
                    };
